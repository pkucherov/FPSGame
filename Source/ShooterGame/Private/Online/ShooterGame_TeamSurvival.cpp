...
#include "Online/ShooterGame_TeamSurvival.h"
#include "Online/ShooterPlayerState.h"
#include "Bots/ShooterAIController.h"

AShooterGame_TeamSurvival::AShooterGame_TeamSurvival(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	NumTeams = 2;
	bDelayedStart = true;
}

void AShooterGame_TeamSurvival::PostLogin(APlayerController* NewPlayer)
{
...
void AShooterGame_TeamSurvival::InitGameState()
{
	Super::InitGameState();
...
bool AShooterGame_TeamSurvival::CanDealDamage(AShooterPlayerState* DamageInstigator, class AShooterPlayerState* DamagedPlayer) const
{
...
int32 AShooterGame_TeamSurvival::ChooseTeam(AShooterPlayerState* ForPlayerState) const
{
...
void AShooterGame_TeamSurvival::DetermineMatchWinner()
{
...
bool AShooterGame_TeamSurvival::IsWinner(AShooterPlayerState* PlayerState) const
{
	return PlayerState && !PlayerState->IsQuitter() && PlayerState->GetTeamNum() == WinnerTeam;
}

bool AShooterGame_TeamSurvival::IsSpawnpointAllowed(APlayerStart* SpawnPoint, AController* Player) const
{
	if (Player)
	{
...
void AShooterGame_TeamSurvival::InitBot(AShooterAIController* AIC, int32 BotNum)
{	
